...
lifted-spark lemma, excitation trichotomy, separation/exponent theorem, and
...
(excitation trichotomy), Theorem~2 (separations and exponents), and
...
\subsection{The excitation trichotomy (main Theorem 1)}
...
\begin{proposition}[(b) arbitrary PSD: only the realized range]\label{prop:psd}
A single covariance $\bm\Sigma_z$ identifies exactly the realized range
$\Rcal(\Scal,\Gam)=\mathrm{im}(\bm\Sigma_z-\sigma_n^2\bm I)
=\mathrm{im}(\bm U_\Scal\Gam^{1/2})\subseteq\mathrm{im}\,\bm U_\Scal$, with
equality $\Rcal=\mathrm{im}\,\bm U_\Scal$ whenever $\Gam\succ0$ (and, when
$\B_{2,\Scal}$ has full column rank so $\bm U_\Scal$ is injective, only then).
Consequences:
\emph{(i)} a full-rank excitation identifies the curl image
$\mathrm{im}\,\B_{2,\Scal}$; \emph{(ii)} a \emph{singular} $\Gam$ can conceal
part of the image and even the support --- there exist $\Scal'\supsetneq\Scal$
and a singular $\Gam'$ with $\bm\Sigma_z(\Scal',\Gam')=\bm\Sigma_z(\Scal,\Gam)$.
The achievable family still obeys
$\{\bm U_\Scal\Gam\bm U_\Scal^{\!\top}:\Gam\succeq0\}
=\{\bm M\succeq0:\mathrm{im}\,\bm M\subseteq\mathrm{im}\,\bm U_\Scal\}$, but
families of nested supports overlap, so this family identity is \emph{not}
support identifiability.
\end{proposition}
\begin{proof}
$\bm\Sigma_z-\sigma_n^2\bm I=\bm U_\Scal\Gam\bm U_\Scal^{\!\top}$ has range
$\mathrm{im}(\bm U_\Scal\Gam^{1/2})$; if $\Gam\succ0$ this is
$\mathrm{im}\,\bm U_\Scal$ (since $\Gam^{1/2}$ is invertible). If
$\bm U_\Scal$ is injective the range has dimension $\rank\Gam$, so equality
forces $\Gam\succ0$; if $\bm U_\Scal$ has a kernel a singular $\Gam$ whose
image complements $\ker\bm U_\Scal$ can still attain $\mathrm{im}\,\bm
U_\Scal$. Family identity:
($\subseteq$) immediate; ($\supseteq$) for PSD $\bm M$ with
$\mathrm{im}\,\bm M\subseteq\mathrm{im}\,\bm U_\Scal$ set
$\Gam=\bm U_\Scal^{+}\bm M(\bm U_\Scal^{+})^{\!\top}\succeq0$, giving
$\bm U_\Scal\Gam\bm U_\Scal^{\!\top}=\bm M$. Witness for (ii): on $K_4$ take
$\Scal=\{\tau_1\}$, $\Scal'=\{\tau_1,\tau_2\}$, $\Gam=[1]$,
$\Gam'=\mathrm{diag}(1,0)$; both give
$\bm\Sigma_z=\sigma_n^2\bm I+\bm u_{\tau_1}\bm u_{\tau_1}^{\!\top}$ although
$\dim\mathrm{im}\,\B_{2,\Scal}=1\ne2=\dim\mathrm{im}\,\B_{2,\Scal'}$
(verified in \texttt{tests/test\_excitation.py}).
\end{proof}
...
Unknown deterministic signals are the degenerate boundary of class (b): for
...
trichotomy and estimator theory: weighted-diagonal identifiability and the
...
favorable-geometry regime of the excitation trichotomy (class-(a) geometry),
...
\item the main paper's vortex-localization study $\leftarrow$
  ...
  ERA5/IBTrACS files).
\end{itemize}
The test-suite (\texttt{pytest -q}, 49 tests) checks: the Fano bounds against
