...
the cyclone vortex-localization study. All statements are validated numerically by the
...
\begin{proposition}[(b) arbitrary PSD: structural characterization]\label{prop:psd}
...
=\begin{cases}
\rho_2^2\,(1+o(1)) & \text{swap (Lemma~\ref{lem:sep16})},\\[2pt]
\tfrac9{16}\rho_2^2\,(1+o(1)) & \text{subset (Lemma~\ref{lem:sep9})},
\end{cases}
\]
where the two forms coincide since $\Delta\bm M=\sigma_c^2\Delta\bm D$
(do \emph{not} write $\rho_2^2\|\Delta\bm M\|_F^2$, which double-counts
$\sigma_c^4$).
