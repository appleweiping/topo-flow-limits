...
the scale of the paper's Fig.~2 ($p=8$) the joint-recovery floor sits well
...
$\mathrm{im}\,\B_2$, $r=\rank\B_2$, $\bm U=\bm Q^{\!\top}\B_2$), the
gradient/harmonic covariance terms are annihilated and
...
Hence the achievable covariance \emph{family} determines, and is determined
by, $\mathrm{im}\,\B_{2,\Scal}$: with unknown unrestricted $\Gam$, supports
are distinguishable as composite hypotheses at most up to equal curl image,
and nothing finer is identifiable. (Pointwise, the image is identified
whenever $\Gam\succ0$; a singular excitation can conceal part of it --- at
$\Gam=\bm0$ all supports coincide.)
\end{proposition}
\begin{proof}
($\subseteq$) immediate. ($\supseteq$) For PSD $\bm M$ with
$\mathrm{im}\,\bm M\subseteq\mathrm{im}\,\bm U_\Scal$, set
$\Gam=\bm U_\Scal^{+}\bm M(\bm U_\Scal^{+})^{\!\top}\succeq0$; then
$\bm U_\Scal\Gam\bm U_\Scal^{\!\top}
=\bm P_{\im\bm U_\Scal}\bm M\bm P_{\im\bm U_\Scal}=\bm M$.
\end{proof}
...
$\mathcal N(\bm 0,\bm\Sigma_z(\Scal'))$. As $\rho_2\to0$,
\[
C_G=\tfrac{1}{16}\,\rho_2^2\,\|\Delta\bm M\|_F^2\,(1+o(1))
=\begin{cases}
...
(test-certified to $3\times10^{-5}$ relative at $\rho_2=10^{-5}$,
...
\texttt{tests/test\_excitation.py} additionally certifies every claim of the
...
\item the main paper's vortex-localization study $\leftarrow$
  ...
  ERA5/IBTrACS files).
\end{itemize}
The test-suite (\texttt{pytest -q}, 46 tests) checks: the Fano bounds against
the empirical recovery boundary of the main paper's Fig.~2 (a converse must
