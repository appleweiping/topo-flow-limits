...
The main paper's Prop.~2 contains a \emph{single-triangle} converse yielding
the floor $\rho^\star(N)=\Theta(\sqrt{\log(1/\delta)/N})$ together with the
Gaussian Fano bound for \emph{joint} recovery of the whole support;
Theorem~\ref{thm:fano-gauss} below proves the latter, and the
signal-agnostic variant extends it.
...
Hence the family of achievable covariances depends on $\Scal$ only through
$\mathrm{im}\,\B_{2,\Scal}$, and with unknown unrestricted $\Gam$ exactly the
curl subspace is identifiable.
\end{proposition}
\begin{proof}
...
(\texttt{test\_exhaustive\_k5\_confuser\_separations}: $45{,}000+$
...
\texttt{test\_subset\_confuser\_exponent\_equals\_isolated\_triangle\_exponent}).
...
$6\%$ relative at $N=40$, zero violations of (i) in $2000$ trials, and the
...
nothing is planted. The Thm.~2 decorrelated score is computed alongside and
...
the main paper's achievability grid and $\alpha$-sweep $\leftarrow$
...
the main paper's vortex-localization study $\leftarrow$ \texttt{run\_real\_cyclone.py}
($\approx$3\,min from the cached ERA5/IBTrACS files).
The test-suite (\texttt{pytest -q}, 45 tests) checks: the Fano bounds against
