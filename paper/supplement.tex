...
lifted-spark lemma and first- vs.\ second-order identifiability dichotomy,
with exhaustive numerical verification; \textbf{S5} construction details of
...
The main paper's Theorem~1(a) is a \emph{single-triangle} converse yielding
the floor $\rho^\star(N)=\Theta(\sqrt{\log(1/\delta)/N})$; its part~(b)
states the Gaussian Fano bound for \emph{joint} recovery of the whole
support, which Theorem~\ref{thm:fano-gauss} below proves, and
the signal-agnostic variant extends.
...
$e^{-NC_G}$ envelope with empirical exponent decreasing onto $C_G$; (vi) exact greedy covariance-atom recovery on rank-deficient $K_5$
(the regime where first-order arguments forbid it).
...
The test-suite (\texttt{pytest -q}, 36 tests) checks: the Fano bounds against
