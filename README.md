...
│
├── tests/                    18 tests — the theorems' guardrails (see §8)
│   ├── test_hodge.py
...
│   ├── figures/              phase_transition/confusability/real_fx/real_traffic .png
...
# 1) validate all theory against Monte-Carlo (18 tests; ~45 s idle, up to ~2 min under load)
.\.venv\Scripts\python.exe -m pytest -q

# 2) regenerate every figure and metric (~5-10 min)
...
```
[1/4] phase transition ...  p=8 active=4; exact-theory rho* at N=90 -> 0.513
[2/4] confusability ...     Hamming@N=200: naive=2.59 whitened=0.00 greedy=0.27
[3/4] real FX ...           curl/grad=1.8e-31 ; K9: 28 curl dims vs 84 triangle coefficients
[4/4] real traffic ...      Anaheim DoF 54/54=1.0 ; recovery@N=95: emp=1.00 theory=1.00
```
...
converse*; the road networks (Figure 4) supply the achievability side —
...
3. **What is (and is not) real in the real-data studies.** FX demonstrates the
   ...
   equilibrium-flow background the detector must be invariant to. Fully-real
...
**复现**：`pytest -q`（18 个测试，空闲约 45 秒、高负载下最多约 2 分钟，
每条定理都对照蒙特卡洛验证）；`python experiments/run_all.py`
（约 5–10 分钟重生成全部图表）；论文在 `paper/main.pdf`
（ICASSP 官方 spconf 格式，4 页正文 + 第 5 页仅参考文献）。全程只需 CPU。
